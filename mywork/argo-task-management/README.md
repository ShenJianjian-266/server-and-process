...
总览：记录任务  任务状态  供Argo回调的接口  重跑接口

确认：已提交|成功|失败  重跑   回调无鉴权方式  重跑复用参数   回调失败兜底：dataserver定时查询，主动查询Argo状态
...
导师提到任务跑完后要让 Argo 调本项目的 HTTP 接口。本项目需要提供一个回调接口，例如：
...
初步思路：在 WorkflowTemplate 中增加一个结束回调步骤，任务完成后由 Argo 调用本项目接口，把 `workflow.name`、`workflow.status`、业务参数等传回来。
...
同时需要改造 `ArgoWorkflowClient`：提交成功拿到 `workflowName` 后，通知任务服务保存记录。更稳妥的方式是让业务调用方传入任务上下文，例如任务类型、dataId、dataName、bizId，而不是只传 Argo 参数。

## 7. 推荐开发顺序

1. 阅读 `ArgoWorkflowClient`，确认当前 submit 请求体和返回值。
2. 梳理所有调用点，明确每类任务能拿到哪些业务字段。
3. 设计任务表字段和状态枚举。
4. 先实现“提交成功后落库”，让页面能查到任务记录。
5. 实现任务分页、详情接口。
6. 实现 Argo 回调接口，先用 Postman 或 curl 模拟回调更新状态。
7. 调研并修改 Argo WorkflowTemplate，让任务结束时自动回调本系统。
8. 实现失败任务重跑接口。
9. 补充单元测试或集成测试，尤其覆盖提交成功、提交失败、回调成功、重复回调、重跑。

## 8. 需要和导师确认的问题

- 任务管理只覆盖数据解析任务，还是覆盖事件解析、手动质检、手动测试提交等所有 Argo 任务？
- 任务状态枚举是否只需要提交中、运行中、成功、失败，还是要保留 Argo 原始状态？
- Web 页面需要哪些筛选条件和展示字段？
- 回调接口是否需要鉴权？如果需要，用固定 token、JWT 还是内网白名单？
- Argo WorkflowTemplate 由谁维护？后端是否可以直接改 `argo/*.yaml`？
- 重跑时是否允许修改参数，还是必须完全复用旧参数？
- 如果 Argo 回调失败，是否需要 Argo 自动重试？
- 是否需要主动轮询 Argo 状态作为回调失败的兜底方案？

## 9. 一句话总结

本任务不是单纯“再调一次 Argo”，而是要把所有 Argo 调用纳入本系统的任务管理：提交时落库、执行中可追踪、结束后可回调更新、失败后可在页面重新提交。
